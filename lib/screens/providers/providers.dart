import 'package:flutter/material.dart';
import '/../main.dart';
import '/models/cartDataModel.dart';



class EligiblityScreenProvider extends ChangeNotifier{
  String _eligiblityMessage = "";
  bool _isEligible =  true;
  int stockqnty =MyApp.Mcartlast.length;


  void addcartdata(String id,String pid, String ptitle,String uprice, String qnty, String tprice, String image){
    CartDataModel model = new CartDataModel();
    model.id = id;
    model.p_id = pid;
    model.ptitle = ptitle;
    model.uprice = uprice;
    model.qnty = qnty;
    model.tprice = tprice;
    model.image = image;
    MyApp.Mcartlast.add(model);
    stockqnty = MyApp.Mcartlast.length;
    print("Function Has been Axecuted  Provider  :"+MyApp.Mcartlast.length.toString());
    notifyListeners();

  }


  void checkEligiblity(int age){
    if(age >= 18)
      eligibleForLicense();
    else
      notEligibleForLicense();
  }

  void eligibleForLicense(){
    _eligiblityMessage = "You are eligible to apply for Driving License";
    _isEligible = true;

    //Call this whenever there is some change in any field of change notifier.
    notifyListeners();
  }


  void notEligibleForLicense(){
    _eligiblityMessage = "You are not eligible to apply for Driving License";
    _isEligible = false;

    //Call this whenever there is some change in any field of change notifier.
    notifyListeners();
  }

  //Getter for Eligiblity message
  String get eligiblityMessage => _eligiblityMessage;

  //Getter for Eligiblity flag
  bool get isEligible => _isEligible;


}